...
public class SecurityConfiguration {
	...
	SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		http.csrf(csrf -> csrf.disable()).cors(Customizer.withDefaults()).authorizeHttpRequests(auth -> auth
				...
				// Vendas consome o catálogo em leitura (listas do formulário + motor de cálculo)
				// e faz o cadastro rápido de cliente do balcão (criação apenas, sem editar/excluir)
				.requestMatchers(HttpMethod.GET, "/api/clientes")
				.hasAnyRole(Permissoes.CLIENTES, Permissoes.VENDAS)
				.requestMatchers(HttpMethod.POST, "/api/clientes/fisico", "/api/clientes/juridico")
				.hasAnyRole(Permissoes.CLIENTES, Permissoes.VENDAS)
				.requestMatchers(HttpMethod.GET, "/api/materias")
				.hasAnyRole(Permissoes.MATERIAS, Permissoes.VENDAS)
				.requestMatchers(HttpMethod.GET, "/api/produtos")
				.hasAnyRole(Permissoes.PRODUTOS, Permissoes.VENDAS)
				.requestMatchers(HttpMethod.POST, "/api/produtos/*/calcular")
				.hasAnyRole(Permissoes.PRODUTOS, Permissoes.VENDAS)
